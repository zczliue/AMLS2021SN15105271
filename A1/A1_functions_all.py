...
from A1 import A1_extract_landmarks as A1_extract
import itertools 
import datetime
import pickle
...
# change the following default settings if the user would like to 
# search for other parameter range
...
def bulk_runtime_estimation(classifier, xtest, clftype):
    ...
    if clftype == 'other':
        ...
        ypred = (classifier.predict(xtest) > 0.5).astype("int32")
        #probability
...
def LogReg_GridSearch_PredictionCV(xtrain, ytrain, xtest, ytest, tuning_parameters, cvfold, num_class, savepath):
    ...
    # confusion matrix
    plot_confusion_matrix(cat_ytrue, ypred_prob, 'LR', savepath)
    # ROC curve
    plot_ROC_curve(cat_ytrue, ypred_prob, num_class, 'LR', savepath)
    ...
    plot_learning_curves_other(lctrain_sizes, lctrain_scores,lcvalid_scores, 'LR', savepath)
...
# When user specify N hidden layers,
# we use a default design that each hidden layer n contains 2^(N+1-n) neuron 
# and 0.5^(n+1) dropout rate
...
# this is to plot learning curve for sequential model for each fold
...
# Plot average learning curve accross all folds for a single MLP para combination
...
# Plot average learning curve accross all folds on 1 plot for all MLP para combinations
...
# Plot learning curves for models other than MLP
...
def true_test_with_opt_clf(te_X, te_Y, optclf):
    ...
    print("True test set Accuracy:", true_test_score)
    print()
    print('Average runtime per test instance:', avg_runtime)
    print('-------------------------------------------------')
...
def true_test_with_opt_seqmodel(te_X, cat_te_Y, Seqclf):
    ...
    print(f'> Avg runtime per test instance: {avg_runtime}')
    print('------------------------------------------------------------------------')
...
def get_A1_results():
    ...
    TaskA1_opt_models_dict = {}
    TaskA1_res_dict = {}

    #create a folder to save generated pickles, clf, images
    savepath = os.path.join('A1', 'TaskA1_res')
    if not os.path.isdir(savepath):
        os.makedirs(savepath)


    print()
    print('-------------------------Reading Task A1 dataset---------------------------')
    ...
    save_model_to_disk(landmark_features, "TaskA1_landmark_features.pickle", savepath)
    save_model_to_disk(label_contents, "TaskA1_label_contents.pickle", savepath)
    save_model_to_disk(no_features_sets, "TaskA1_no_features_sets.pickle", savepath)
    ...
    print()
    print('-------------------------Task A1: Gird searching on Logistic regression---------------------------')
    Logclf, LR_scores_on_best_para = LogReg_GridSearch_PredictionCV(tr_X, tr_Y, te_X, te_Y, LogReg_tuned_parameters, cvfold, num_class, savepath)
    TaskA1_opt_models_dict['LR'] = Logclf
    TaskA1_res_dict['LR'] = LR_scores_on_best_para
    print()


    # SVM result:
    print()
    print('-------------------------Task A1: Gird searching on SVM---------------------------')
    ...
    TaskA1_opt_models_dict['SVM'] = SVMclf
    TaskA1_res_dict['SVM'] = SVM_scores_on_best_para
    print()

    # Random Forest result:
    print()
    print('-------------------------Task A1: Gird searching on Rando Forest---------------------------')
    ...
    TaskA1_opt_models_dict['RF'] = RFclf
    TaskA1_res_dict['RF'] = RF_scores_on_best_para
    print()

    # KNN resulS:
    print()
    print('-------------------------Task A1: Gird searching on KNN------------------------------')
    ...
    TaskA1_opt_models_dict['KNN'] = KNNclf
    TaskA1_res_dict['KNN'] = KNN_scores_on_best_para
    print()

    #Sequential model results:
    print()
    print('-------------------------Task A1: Gird searching on MLP------------------------------')
    ...
    TaskA1_opt_models_dict['MLP'] = Seqmodel
    TaskA1_res_dict['MLP'] = Seq_pseudo_test_score
    print()


    #save res for future use (MLP models cannot be pickled so we do not save)
    #save_model_to_disk(TaskA1_opt_models_dict, "TaskA1_opt_models_dict.pkl", savepath)
    save_model_to_disk(TaskA1_res_dict, "TaskA1_res_dict.pkl", savepath)

    #save trained models wit optimum para set after searching
    save_model_to_disk(Logclf, "TaskA1_Logclf.pkl", savepath)
    save_model_to_disk(SVMclf, "TaskA1_SVMclf.pkl", savepath)
    save_model_to_disk(RFclf, "TaskA1_RFclf.pkl", savepath)
    save_model_to_disk(KNNclf, "TaskA1_KNNclf.pkl", savepath)

    Seqmodel.save(os.path.join(savepath, "TaskA1_Seqmodel"))



    return TaskA1_opt_models_dict, TaskA1_res_dict





def get_A1_true_test_res(true_test_image_folder, true_test_label_name, model_dict, use_pretrained):


    TaskA1_true_test_res_dict = {}

    loadpath = os.path.join('A1', 'TaskA1_pretrained')
    ...
    #68 feature contents (from image), corresponding label (from csv), the image name where dlib cannot extract features
    landmark_features, label_contents, no_features_sets = A1_extract.extract_features_labels(true_test_image_folder, true_test_label_name)
    #number of unique classes to be predicted
    num_class = len(np.unique(label_contents))
    ...
    #save read dataset for future use
    save_model_to_disk(landmark_features, "TaskA1_true_landmark_features.pickle", savepath)
    save_model_to_disk(label_contents, "TaskA1_true_label_contents.pickle", savepath)
    save_model_to_disk(no_features_sets, "TaskA1_true_no_features_sets.pickle", savepath)
    ...
    if use_pretrained == True:
        ...
        #load models
        Logclf = load_model_from_disk("TaskA1_Logclf.pkl", loadpath, 'pickle')
        SVMclf = load_model_from_disk("TaskA1_SVMclf.pkl", loadpath, 'pickle')
        RFclf = load_model_from_disk("TaskA1_RFclf.pkl", loadpath, 'pickle')
        KNNclf = load_model_from_disk("TaskA1_KNNclf.pkl", loadpath, 'pickle')
        Seqmodel = load_model_from_disk("TaskA1_Seqmodel", loadpath, 'Seq')

    else:    
        #opt models from grid search:
        Logclf = model_dict['LR']
    ...
    #
    print()
    print('-------------------------Task A1: True test performance with LR------------------------------')
    LR_true_test_score = true_test_with_opt_clf(true_testX, true_testY, Logclf)
    TaskA1_true_test_res_dict['LR'] = {}
    TaskA1_true_test_res_dict['LR']['true_test_score'] = LR_true_test_score

    print()
    print('-------------------------Task A1: True test performance with SVM------------------------------')
    SVM_true_test_score = true_test_with_opt_clf(true_testX, true_testY, SVMclf)
    TaskA1_true_test_res_dict['SVM'] = {}
    TaskA1_true_test_res_dict['SVM']['true_test_score'] = SVM_true_test_score

    print()
    print('-------------------------Task A1: True test performance with RF------------------------------')
    RF_true_test_score = true_test_with_opt_clf(true_testX, true_testY, RFclf)
    TaskA1_true_test_res_dict['RF'] = {}
    TaskA1_true_test_res_dict['RF']['true_test_score'] = RF_true_test_score

    print()
    print('-------------------------Task A1: True test performance with KNN------------------------------')
    KNN_true_test_score = true_test_with_opt_clf(true_testX, true_testY, KNNclf)
    TaskA1_true_test_res_dict['KNN'] = {}
    TaskA1_true_test_res_dict['KNN']['true_test_score'] = KNN_true_test_score

    print()
    print('-------------------------Task A1: True test performance with MLP------------------------------')
    Seq_true_test_res_score = true_test_with_opt_seqmodel(true_testX, cat_true_testY, Seqmodel)
    TaskA1_true_test_res_dict['MLP'] = {}
    TaskA1_true_test_res_dict['MLP']['true_test_score'] = Seq_true_test_res_score 


    save_model_to_disk(TaskA1_true_test_res_dict, "TaskA1_true_test_res_dict.pkl", savepath)

    return TaskA1_true_test_res_dict
