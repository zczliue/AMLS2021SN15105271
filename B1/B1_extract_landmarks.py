...
predictor = dlib.shape_predictor(os.path.join('B1', 'shape_predictor_68_face_landmarks.dat'))
...
def get_jawline(landmark_features):

    #jawline landmark
    rt_jawline_landmarks = []

    for item in landmark_features:
        (i,j) = FACIAL_LANDMARKS_68_IDXS["jaw"]
        pts = item[i:j]
        rt_jawline_landmarks.append(pts)

    jawline_landmarks = np.array(rt_jawline_landmarks)

    return jawline_landmarks