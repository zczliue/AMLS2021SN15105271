...
# This is the function file for Task B2,
...
from B2 import B2_preprocess as B2_prep
import itertools 
import datetime
import pickle
...
#------------------------------------------Task B2 function definition-----------------------------------------
#-------------------------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------------------------

# This function for B2 is slightly from previous split functions:
...
# The main difference is that we do not need to reshape X since the input values are already in proper shape
...
#return both predict prob and predict labels
...
def get_B2_results():
    ...
    TaskB2_opt_models_dict = {}
    TaskB2_res_dict = {}

    #create a folder to save generated pickles, clf, images
    savepath = os.path.join('B2', 'TaskB2_res')
    if not os.path.isdir(savepath):
        os.makedirs(savepath)


    print()
    print('-------------------------Reading Task B2 dataset---------------------------')
    #preprocess the dataset to extract dominant rgb, landmarks and labels

    #landmark_features: 68 feature contents (from image), 
    #label_contents: label (from csv), 
    #no_features_sets: the image name where dlib cannot extract features
    #identified_file_sets: the file names that can be detected by dlib
    #dominant_rgbs: dominant 3 rgb colours for eyes in each image
    landmark_features, label_contents, no_features_sets, identified_file_sets, dominant_rgbs  =  B2_prep.extract_features_labels_and_dominant_colour(image_folder, label_name, roi_margin, n_colors)
    ...
    #instead of using 68*2 = 136 features we are now only using 3 features (rgb) value for each sample
    tr_X, te_X, tr_Y, te_Y, cat_tr_Y, cat_te_Y  = split_data(dominant_rgbs, label_contents, split_ratio)    

    #save read dataset for future use
    save_model_to_disk(landmark_features, "TaskB2_landmark_features.pickle", savepath)
    save_model_to_disk(label_contents, "TaskB2_label_contents.pickle", savepath)
    save_model_to_disk(no_features_sets, "TaskB2_no_features_sets.pickle", savepath)
    save_model_to_disk(dominant_rgbs, "TaskB2_dominant_rgbs.pickle", savepath)
    ...
    # SVM result:
    print()
    print('-------------------------Task B2: Gird searching on SVM---------------------------')
    ...
    TaskB2_opt_models_dict['SVM'] = SVMclf
    TaskB2_res_dict['SVM'] = SVM_scores_on_best_para
    print()

    # Random Forest result:
    print()
    print('-------------------------Task B2: Gird searching on Rando Forest---------------------------')
    ...
    TaskB2_opt_models_dict['RF'] = RFclf
    TaskB2_res_dict['RF'] = RF_scores_on_best_para
    print()

    # KNN resulS:
    print()
    print('-------------------------Task B2: Gird searching on KNN------------------------------')
    ...
    TaskB2_opt_models_dict['KNN'] = KNNclf
    TaskB2_res_dict['KNN'] = KNN_scores_on_best_para
    print()

    #Sequential model results:
    print()
    print('-------------------------Task B2: Gird searching on MLP------------------------------')
    ...
    TaskB2_opt_models_dict['MLP'] = Seqmodel
    TaskB2_res_dict['MLP'] = Seq_pseudo_test_score
    print()


    #save res for future use (MLP models cannot be pickled so we do not save)
    save_model_to_disk(TaskB2_res_dict, "TaskB2_res_dict.pkl", savepath)

    #save trained models wit optimum para set after searching
    save_model_to_disk(SVMclf, "TaskB2_SVMclf.pkl", savepath)
    save_model_to_disk(RFclf, "TaskB2_RFclf.pkl", savepath)
    save_model_to_disk(KNNclf, "TaskB2_KNNclf.pkl", savepath)

    Seqmodel.save(os.path.join(savepath, "TaskB2_Seqmodel"))


    return TaskB2_opt_models_dict, TaskB2_res_dict




def get_B2_true_test_res(true_test_image_folder, true_test_label_name, model_dict, use_pretrained):

    TaskB2_true_test_res_dict = {}

    loadpath = os.path.join('B2', 'TaskB2_pretrained')
    ...
    landmark_features, label_contents, no_features_sets, identified_file_sets, dominant_rgbs  =  B2_prep.extract_features_labels_and_dominant_colour(true_test_image_folder, true_test_label_name, roi_margin, n_colors)
    #number of unique classes to be predicted
    num_class = len(np.unique(label_contents))
    ...
    #save read dataset for future use
    save_model_to_disk(landmark_features, "TaskB2_true_landmark_features.pickle", savepath)
    save_model_to_disk(label_contents, "TaskB2_true_label_contents.pickle", savepath)
    save_model_to_disk(no_features_sets, "TaskB2_true_no_features_sets.pickle", savepath)
    save_model_to_disk(dominant_rgbs, "TaskB2_true_dominant_rgbs.pickle", savepath)

    #normaisation and convert to categorical matrix
    true_testX, true_testY, cat_true_testY = convert_true_test_data(dominant_rgbs, label_contents)

    #use pretrained models
    if use_pretrained == True:
        #load models
        SVMclf = load_model_from_disk("TaskB2_SVMclf.pkl", loadpath, 'pickle')
        RFclf = load_model_from_disk("TaskB2_RFclf.pkl", loadpath, 'pickle')
        KNNclf = load_model_from_disk("TaskB2_KNNclf.pkl", loadpath, 'pickle')
        Seqmodel = load_model_from_disk("TaskB2_Seqmodel", loadpath, 'Seq')
    ...
    #
    print()
    print('-------------------------Task B2: True test performance with SVM------------------------------')
    SVM_true_test_score = true_test_with_opt_clf(true_testX, true_testY, SVMclf)
    TaskB2_true_test_res_dict['SVM'] = {}
    TaskB2_true_test_res_dict['SVM']['true_test_score'] = SVM_true_test_score

    print()
    print('-------------------------Task B2: True test performance with RF------------------------------')
    RF_true_test_score = true_test_with_opt_clf(true_testX, true_testY, RFclf)
    TaskB2_true_test_res_dict['RF'] = {}
    TaskB2_true_test_res_dict['RF']['true_test_score'] = RF_true_test_score

    print()
    print('-------------------------Task B2: True test performance with KNN------------------------------')
    KNN_true_test_score = true_test_with_opt_clf(true_testX, true_testY, KNNclf)
    TaskB2_true_test_res_dict['KNN'] = {}
    TaskB2_true_test_res_dict['KNN']['true_test_score'] = KNN_true_test_score

    print()
    print('-------------------------Task B2: True test performance with MLP------------------------------')
    Seq_true_test_res_score = true_test_with_opt_seqmodel(true_testX, cat_true_testY, Seqmodel)
    TaskB2_true_test_res_dict['MLP'] = {}
    TaskB2_true_test_res_dict['MLP']['true_test_score'] = Seq_true_test_res_score 


    save_model_to_disk(TaskB2_true_test_res_dict, "TaskB2_true_test_res_dict.pkl", savepath)

    return TaskB2_true_test_res_dict
