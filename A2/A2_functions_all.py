...
# This is the function file for Task A2.
# The main functions are exactly same as A1
# the only change is the extracted label_name is now 'smiling'
...
from A2 import A2_extract_landmarks as A2_extract
import itertools 
import datetime
import pickle
...
def get_A2_results():
    ...
    TaskA2_opt_models_dict = {}
    TaskA2_res_dict = {}

    #create a folder to save generated pickles, clf, images
    savepath = os.path.join('A2', 'TaskA2_res')
    if not os.path.isdir(savepath):
        os.makedirs(savepath)


    print()
    print('-------------------------Reading Task A2 dataset---------------------------')
    #68 feature contents (from image), corresponding label (from csv), the image name where dlib cannot extract features
    landmark_features, label_contents, no_features_sets = A2_extract.extract_features_labels(image_folder, label_name)
    ...
    save_model_to_disk(landmark_features, "TaskA2_landmark_features.pickle", savepath)
    save_model_to_disk(label_contents, "TaskA2_label_contents.pickle", savepath)
    save_model_to_disk(no_features_sets, "TaskA2_no_features_sets.pickle", savepath)
    ...
    print()
    print('-------------------------Task A2: Gird searching on Logistic regression---------------------------')
    ...
    TaskA2_opt_models_dict['LR'] = Logclf
    TaskA2_res_dict['LR'] = LR_scores_on_best_para
    print()


    # SVM result:
    print()
    print('-------------------------Task A2: Gird searching on SVM---------------------------')
    ...
    TaskA2_opt_models_dict['SVM'] = SVMclf
    TaskA2_res_dict['SVM'] = SVM_scores_on_best_para
    print()

    # Random Forest result:
    print()
    print('-------------------------Task A2: Gird searching on Rando Forest---------------------------')
    ...
    TaskA2_opt_models_dict['RF'] = RFclf
    TaskA2_res_dict['RF'] = RF_scores_on_best_para
    print()

    # KNN resulS:
    print()
    print('-------------------------Task A2: Gird searching on KNN------------------------------')
    ...
    TaskA2_opt_models_dict['KNN'] = KNNclf
    TaskA2_res_dict['KNN'] = KNN_scores_on_best_para
    print()

    #Sequential model results:
    print()
    print('-------------------------Task A2: Gird searching on MLP------------------------------')
    ...
    TaskA2_opt_models_dict['MLP'] = Seqmodel
    TaskA2_res_dict['MLP'] = Seq_pseudo_test_score
    print()


    #save res for future use (MLP models cannot be pickled so we do not save)
    save_model_to_disk(TaskA2_res_dict, "TaskA2_res_dict.pkl", savepath)

    #save trained models wit optimum para set after searching
    save_model_to_disk(Logclf, "TaskA2_Logclf.pkl", savepath)
    save_model_to_disk(SVMclf, "TaskA2_SVMclf.pkl", savepath)
    save_model_to_disk(RFclf, "TaskA2_RFclf.pkl", savepath)
    save_model_to_disk(KNNclf, "TaskA2_KNNclf.pkl", savepath)

    Seqmodel.save(os.path.join(savepath, "TaskA2_Seqmodel"))




    return TaskA2_opt_models_dict, TaskA2_res_dict





def get_A2_true_test_res(true_test_image_folder, true_test_label_name, model_dict, use_pretrained):

    TaskA2_true_test_res_dict = {}


    loadpath = os.path.join('A2', 'TaskA2_pretrained')
    ...
    #68 feature contents (from image), corresponding label (from csv), the image name where dlib cannot extract features
    landmark_features, label_contents, no_features_sets = A2_extract.extract_features_labels(true_test_image_folder, true_test_label_name)
    #number of unique classes to be predicted
    num_class = len(np.unique(label_contents))
    ...
    #save read dataset for future use
    save_model_to_disk(landmark_features, "TaskA2_true_landmark_features.pickle", savepath)
    save_model_to_disk(label_contents, "TaskA2_true_label_contents.pickle", savepath)
    save_model_to_disk(no_features_sets, "TaskA2_true_no_features_sets.pickle", savepath)
    ...
    if use_pretrained == True:
        ...
        #load models
        Logclf = load_model_from_disk("TaskA2_Logclf.pkl", loadpath, 'pickle')
        SVMclf = load_model_from_disk("TaskA2_SVMclf.pkl", loadpath, 'pickle')
        RFclf = load_model_from_disk("TaskA2_RFclf.pkl", loadpath, 'pickle')
        KNNclf = load_model_from_disk("TaskA2_KNNclf.pkl", loadpath, 'pickle')
        Seqmodel = load_model_from_disk("TaskA2_Seqmodel", loadpath, 'Seq')
    ...
    #
    print()
    print('-------------------------Task A2: True test performance with LR------------------------------')
    LR_true_test_score = true_test_with_opt_clf(true_testX, true_testY, Logclf)
    TaskA2_true_test_res_dict['LR'] = {}
    TaskA2_true_test_res_dict['LR']['true_test_score'] = LR_true_test_score

    print()
    print('-------------------------Task A2: True test performance with SVM------------------------------')
    SVM_true_test_score = true_test_with_opt_clf(true_testX, true_testY, SVMclf)
    TaskA2_true_test_res_dict['SVM'] = {}
    TaskA2_true_test_res_dict['SVM']['true_test_score'] = SVM_true_test_score

    print()
    print('-------------------------Task A2: True test performance with RF------------------------------')
    RF_true_test_score = true_test_with_opt_clf(true_testX, true_testY, RFclf)
    TaskA2_true_test_res_dict['RF'] = {}
    TaskA2_true_test_res_dict['RF']['true_test_score'] = RF_true_test_score

    print()
    print('-------------------------Task A2: True test performance with KNN------------------------------')
    KNN_true_test_score = true_test_with_opt_clf(true_testX, true_testY, KNNclf)
    TaskA2_true_test_res_dict['KNN'] = {}
    TaskA2_true_test_res_dict['KNN']['true_test_score'] = KNN_true_test_score

    print()
    print('-------------------------Task A2: True test performance with MLP------------------------------')
    Seq_true_test_res_score = true_test_with_opt_seqmodel(true_testX, cat_true_testY, Seqmodel)
    TaskA2_true_test_res_dict['MLP'] = {}
    TaskA2_true_test_res_dict['MLP']['true_test_score'] = Seq_true_test_res_score 


    save_model_to_disk(TaskA2_true_test_res_dict, "TaskA2_true_test_res_dict.pkl", savepath)

    return TaskA2_true_test_res_dict
